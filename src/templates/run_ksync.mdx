...
<CodeBlock language="bash">
{`ksync serve-snapshots --binary="/path/to/${pool().binaryName}" --home="/path/to/${pool().configName}" --snapshot-pool-id=${pool().networks["Mainnet"]} --block-pool-id=${pool().blockPoolId}`}
</CodeBlock>
