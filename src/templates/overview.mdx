  ...
  if (pool().runtime.includes("bsync")) {
    return (
      <div>
        ...
        makes them permanently available with Arweave and Bundlr
      </div>
    );
  }
  return (
    <div>
      ...
      {pool().name} and makes them permanently available with Arweave and Bundlr
    </div>
  );
};
