...
<CodeBlock language="bash">
{pool().genesisFile.endsWith(".gz") ? `wget -qO- ${pool().genesisFile} | zcat > ~/.${pool().val_name}/config/genesis.json` : `wget -qO- ${pool().genesisFile} | cat > ~/.${pool().val_name}/config/genesis.json`}
</CodeBlock>
