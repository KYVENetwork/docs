...
- `data_size`: the amount of bytes the data has which got stored on the storage provider. Used for
  the [reward calculation](/docs/learn/protocol-structure/advanced-concepts/uploader-reward-calculation.md) and is also verified by
  other validators
...
mentioned before
in [Storage Pools](/docs/learn/protocol-structure/general-concepts/storage-pools.md) there are two endpoints where pools can be queried:
