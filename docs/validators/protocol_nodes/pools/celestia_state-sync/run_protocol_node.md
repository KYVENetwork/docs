...
Finally the protocol validator can be started. In order to start it, execute the following command. Since we named our valaccount `celestia-ssync` we select the this valaccount to start the KYSOR.
...
```bash
./kysor start --valaccount 'celestia-ssync'
```

You can also start the process in debug mode by adding the `--debug` flag like this:

```bash
./kysor start --valaccount 'celestia-ssync' --debug
```
...
Since the KYSOR can run on multiple pools on one machine we would recommend naming the daemon service after the valaccount name and with a `d` appending to it. With that you can create multiple service files and control each of them. This example shows the service file for our valaccount `celestia-ssync`
...
[Service]
User=$USER
ExecStart=/home/$USER/kysor start --valaccount celestia-ssync
Restart=on-failure
RestartSec=3
