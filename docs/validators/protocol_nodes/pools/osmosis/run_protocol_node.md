...
With the Osmosis node in place and the KYVE node also installed you can finally start the protocol node.

## Start Node

Finally the protocol validator can be started. In order to start it, execute the following command. Since we named our valaccount `osmosis` we select the this valaccount to start the KYSOR.

```bash
./kysor start --valaccount 'osmosis'
```

You can also start the process in debug mode by adding the `--debug` flag like this:

```bash
./kysor start --valaccount 'osmosis' --debug
```

:::tip
If you're running the Osmosis node on a separate machine you have to tell the protocol validator with an env variable the new rpc endpoint.
...
```bash
./kysor start --valaccount 'osmosis' --env-file="path/to/.env"
```
...
2023-02-13 08:46:00.873  INFO  Pool ID = 1
...
Since the KYSOR can run on multiple pools on one machine we would recommend naming the daemon service after the valaccount name and with a `d` appending to it. With that you can create multiple service files and control each of them. This example shows the service file for our valaccount `osmosis`
...
[Service]
User=$USER
ExecStart=/home/$USER/kysor start --valaccount osmosisxs
Restart=on-failure
RestartSec=3
...
```
sudo journalctl -u osmosis -f -o cat
```
