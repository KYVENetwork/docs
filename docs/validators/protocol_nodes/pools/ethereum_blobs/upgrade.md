...
```json
{
  ...
  "duration": "600",
  "binaries": "{\"kyve-linux-arm64\":\"https://github.com/KYVENetwork/kyvejs/releases/download/%40kyvejs%2Fetherum-blobs%401.0.0-beta.1/kyve-linux-arm64.zip\",\"kyve-linux-x64\":\"https://github.com/KYVENetwork/kyvejs/releases/download/%40kyvejs%2Fetherum-blobs%401.0.0-beta.1/kyve-linux-x64.zip\",\"kyve-macos-x64\":\"https://github.com/KYVENetwork/kyvejs/releases/download/%40kyvejs%2Fetherum-blobs%401.0.0-beta.1/kyve-macos-x64.zip\"}"
}
```

Here **every** pool which is running on the _@kyvejs/etherum-blobs_ runtime gets upgraded with the new version of _1.0.0-beta.1_. The upgrade will go into effect
...
:::tip
**INFO**: Checkout the correct version in kyvejs like this: `git checkout tags/@kyvejs/etherum-blobs@x.x.x -b x.x.x`. More info on building binaries from source can be found [here](/validators/protocol_nodes/pools/archway/installation.md#build-from-source)
:::
...
```bash
@kyvejs/etherum-blobs version: 1.0.0-beta.1
...
The version of _@kyvejs/etherum-blobs_ should match with the version in the upgrade proposal. Once the upgrade executed the old binary will automatically stop with an error since
