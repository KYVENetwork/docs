...
Finally the protocol validator can be started. In order to start it, execute the following command. Since we named our valaccount `noble-ssync` we select the this valaccount to start the KYSOR.
...
Since the KYSOR can run on multiple pools on one machine we would recommend naming the daemon service after the valaccount name and with a `d` appending to it. With that you can create multiple service files and control each of them. This example shows the service file for our valaccount `noble-ssync`
...
[Service]
User=$USER
ExecStart=/home/$USER/kysor start --valaccount noble-ssync
Restart=on-failure
RestartSec=3
