...
With the Celestia node in place and the KYVE node also installed you can finally start the protocol node.

## Start Node

Finally the protocol validator can be started. In order to start it, execute the following command. Since we named our valaccount `celestia` we select the this valaccount to start the KYSOR.

```bash
./kysor start --valaccount 'celestia'
```

You can also start the process in debug mode by adding the `--debug` flag like this:

```bash
./kysor start --valaccount 'celestia' --debug
```

:::tip
If you're running the Celestia node on a separate machine you have to tell the protocol validator with an env variable the new rpc endpoint.
...
```bash
./kysor start --valaccount 'celestia' --env-file="path/to/.env"
```
...
A dialog should open where you should select the pool you want to join (here Celestia). After that enter the valaddress that needs to be authorized and the valname, which just serves as a security that the node has actually been started. (If you join a pool without having the node running you are in danger of receiving a timeout slash because once you join a pool you are expected to validate and upload data).
...
Since the KYSOR can run on multiple pools on one machine we would recommend naming the daemon service after the valaccount name and with a `d` appending to it. With that you can create multiple service files and control each of them. This example shows the service file for our valaccount `celestia`
...
[Service]
User=$USER
ExecStart=/home/$USER/kysor start --valaccount celestia
Restart=on-failure
RestartSec=3
