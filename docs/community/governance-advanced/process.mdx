...
Deposit amounts are at risk of being burned. Prior to a governance proposal entering the voting period (ie. for the proposal to be voted upon), there must be at least a minimum number of KYVE deposited (<GovParam render={params => <span>{DepositToKYVE(params.params.min_deposit)}</span>} />/<GovParam render={params => <span>{DepositToKYVE(params.params.expedited_min_deposit)}</span>} /> for expedited proposal). Anyone may contribute to this deposit. Deposits of passed and failed proposals are returned to the contributors.

In the past, different people have considered contributions amounts differently. There is some consensus that this should be a personal choice. There is also some consensus that this can be an opportunity for supporters to signal their support by adding to the deposit amount, so a proposer may choose to leave contribution room (ie. a deposit below <GovParam render={params => <span>{DepositToKYVE(params.params.min_deposit)}</span>} />) so that others may participate. It is important to remember that any contributed KYVE are at risk of being burned.

### Burned deposits

Deposits are burned when proposals:

1. **Are vetoed** - deposits for proposals with <GovParam render={params => <span>{StringToPercent(params.params.veto_threshold)}</span>} /> of voting power backing the `NoWithVeto` option are also burned
...
3. A simple majority (greater than <GovParam render={params => <span>{StringToPercent(params.params.threshold)}</span>} />) of the participating voting power must back the `Yes` vote during the <GovParam render={params => <span>{StringToDays(params.params.voting_period)}</span>} />-day voting period
...
Voting power is determined by stake weight at the end of the <GovParam render={params => <span>{StringToDays(params.params.voting_period)}</span>} />-day voting period and is proportional to the number of total KYVE participating in the vote. Only bonded KYVE count towards the voting power for a governance proposal. Liquid KYVE will not count toward a vote or quorum.
...
Though a simple majority `Yes` vote (ie. 50% of participating voting power) is required for a governance proposal vote to pass, a `NoWithVeto` vote of <GovParam render={params => <span>{StringToPercent(params.params.veto_threshold)}</span>} />  of participating voting power or greater can override this outcome and cause the proposal to fail. This enables a minority group representing greater than <GovParam render={params => <span>{StringToPercent(params.params.veto_threshold)}</span>} /> of voting power to fail a proposal that would otherwise pass.

### How is quorum determined?

Voting power, whether backing a vote of `Yes`, `Abstain`, `No`, or `NoWithVeto`, counts toward quorum. Quorum is required for the outcome of a governance proposal vote to be considered valid and for deposit contributors to recover their deposit amounts. If the proposal vote does not reach quorum (ie. less than 33.4% of the network's voting power is participating) within <GovParam render={params => <span>{StringToDays(params.params.voting_period)}</span>} /> days, any deposit amounts will be burned and the proposal outcome will not be considered to be valid.
